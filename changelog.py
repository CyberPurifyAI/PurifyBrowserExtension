...
if __name__ == '__main__':
    ...
    merged_branches = subprocess.check_output(
        'git log --oneline --grep="to master" --fixed-strings --reverse --after=' + args.after_date, shell=True)
    ...
    for line in merged_branches.splitlines():
        ...
        if branch_name_parts:
            if not str(branch_name_parts.group(3), 'utf-8').isnumeric():
                continue

            summary = get_gh_issue_summary(
                branch_name_parts, args.repo, args.token)
