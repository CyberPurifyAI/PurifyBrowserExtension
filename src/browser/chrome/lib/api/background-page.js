/**
 * ----------------------------------------------------------------------------------
 * PurifyBrowserExtension background-page.js
...
(function (purify, browser) {
  ...
  function getRequestDetails(details) {
     ...
     * https://github.com/CyberPurify/PurifyBrowserExtension/issues/777
     */
  ...
  if (
    typeof browser.webRequest.OnHeadersReceivedOptions !== "undefined" &&
  ...
  const onBeforeSendHeaders = {
    ...
    addListener(callback, urls) {
       ...
       * https://github.com/CyberPurify/PurifyBrowserExtension/issues/1634
       * https://github.com/CyberPurify/PurifyBrowserExtension/issues/1644
      ...
      if (
        ...
        const extraHeadersTypes = allTypes.filter(
          (type) => !nonExtraHeadersTypes.includes(type)
        );
  ...
  purify.app = {
     ...
     * https://github.com/CyberPurify/PurifyBrowserExtension/issues/1437
     * @param referrerUrl
     * @returns {boolean}
     */
  ...
  purify.webRequest = {
    ...
    onBeforeRedirect,
    webSocketSupported:
      typeof browser.webRequest.ResourceType !== "undefined" &&
  ...
  const onCreatedNavigationTarget = {
    addListener(callback) {
      ...
      if (
        typeof browser.webNavigation.onCreatedNavigationTarget === "undefined"
      ) {
        return;
      }
  ...
  const browserActionSupported =
    typeof browser.browserAction.setIcon !== "undefined";
