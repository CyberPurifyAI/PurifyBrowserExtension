...
const WhiteListFilter = function(options) {
    ...
    CheckboxUtils.updateCheckbox(
        [changeDefaultWhiteListModeCheckbox], !options.defaultWhiteListMode
    );
...
const AntiBannerFilters = function(options) {
    ...
    function generateFiltersNamesDescription(filters) {
        ...
        if (length > namesDisplayCount) {
            ...
            enabledFiltersNamesString = i18n.getMessage(
                "options_filters_enabled_and_more_divider", [displayNamesString, (length - namesDisplayCount).toString()]
            );
        } else if (length > 1) {
            const lastName = enabledFiltersNames.slice(length - 1)[0];
            const firstNames = enabledFiltersNames.slice(0, length - 1);
            enabledFiltersNamesString = i18n.getMessage(
                "options_filters_enabled_and_divider", [firstNames.join(", "), lastName]
            );
        } else if (length === 1) {
    ...
    const getDisplayOptionValue = (selectionNode) => [].slice.call(selectionNode.options).filter((o) => o.selected)[0].value;
