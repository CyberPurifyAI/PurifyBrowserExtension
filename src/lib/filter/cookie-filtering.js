 ...
 */
purify.cookieFiltering = (function (purify) {
  "use strict";
   ...
   */
  const scheduleProcessingCookie = (
    requestId,
    name,
    url,
    thirdParty,
  ...
  const updateSetCookieMaxAge = (setCookie, maxAge) => {
    ...
    if (
      cookieExpiresTimeSec === null ||
      cookieExpiresTimeSec > newCookieExpiresTimeSec
    ) {
  ...
  const modifySetCookieByRules = (setCookie, rules) => {
    ...
    for (let i = 0; i < rules.length; i += 1) {
      ...
      if (
        typeof cookieOption.maxAge === "number" &&
   ...
   */
  const processModifySetCookieByRules = (
    tab,
    setCookie,
    cookieDomain,
    thirdParty,
  ...
  const filterRequestHeaders = function (requestId, requestHeaders) {
    ...
    const requestType = context.requestType || purify.RequestTypes.DOCUMENT;

    const cookieHeader = purify.utils.browser.findHeaderByName(
      requestHeaders,
      "Cookie"
    );
    const cookies = purify.utils.cookie.parseCookie(
      cookieHeader ? cookieHeader.value : null
    );
    if (!cookies) {
      return false;
    }
    ...
    const rules = purify.webRequestService.getCookieRules(
      tab,
      requestUrl,
      referrerUrl,
      requestType
  ...
  const filterResponseHeaders = function (requestId, responseHeaders) {
    ...
    const referrerUrl = context.originUrl || context.referrerUrl;
    const requestType = context.requestType || purify.RequestTypes.DOCUMENT;
    const requestHost = purify.utils.url.getHost(requestUrl);
    ...
    while (iResponseHeaders > 0) {
      ...
      const setCookie = purify.utils.cookie.parseSetCookie(header.value);
      if (!setCookie) {
        continue;
      }
      ...
      const thirdParty =
        referrerUrl &&
        purify.utils.url.isThirdPartyRequest(cookieUrl, referrerUrl);
      const rules = purify.webRequestService.getCookieRules(
        tab,
        cookieUrl,
        referrerUrl,
  ...
  const modifyCookies = (requestId) => {
    ...
    if (purify.frames.shouldStopRequestProcess(tab)) {
      purify.console.debug("Tab is whitelisted or protection is disabled");
      cookiesMap.delete(requestId);
      return false;
    }
