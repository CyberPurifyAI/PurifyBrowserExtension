...
import { PURIFY_CLASSES } from "./purify_classes";
...
interface purifyjsOptions {
  size?: number;
  type?: string;
}

export type predictionType = {
  className: typeof PURIFY_CLASSES[keyof typeof PURIFY_CLASSES]
  probability: number
}
...
const BASE_PATH =
  ...
  // "https://storage.googleapis.com/purify/tfjs_purify_mobilenet/";
  "https://storage.googleapis.com/purify/purify_mobilenet_tfjs/";
...
export class PURIFYJS {
  public endpoints: string[];
  public model: tf.LayersModel | tf.GraphModel;

  private options: purifyjsOptions;
...
async function getTopKClasses(
  ...
  for (let i = 0; i < topkIndices.length; i++) {
    topClassesAndProbs.push({
      className: PURIFY_CLASSES[topkIndices[i]],
