 ...
 */
export async function load(threshold: number, toxicityLabels: string[], toxicityModelPath: string) {
    const model = new ToxicityClassifier(threshold, toxicityLabels, toxicityModelPath);
    await model.load();
    return model;
}
...
export class ToxicityClassifier {
    ...
    private toxicityModelPath: string;

    constructor(threshold = 0.85, toxicityLabels: string[] = [], toxicityModelPath: string = '') {
        this.threshold = threshold;
        this.toxicityLabels = toxicityLabels;
        this.toxicityModelPath = toxicityModelPath;
    }

    async loadModel() {
        // console.log('this.toxicityModelPath', this.toxicityModelPath);
        return tfconv.loadGraphModel(this.toxicityModelPath, { fromTFHub: false });
    }
