...
const paths = {
    ...
    dest: path.join(
        BUILD_DIR,
        BRANCH,
        BRANCH === BRANCH_DEV ? "firefox-amo" : `firefox-amo-${BRANCH}-unsigned`
    ),
};
...
const localesProcess = (done) =>
    updateLocalesMSGName(BRANCH, paths.dest, done, FIREFOX_WEBEXT);
...
const createArtifactBuild = (done) => {
    ...
    return gulp
        .src(dest.inner)
        .pipe(zip(`firefox-${BRANCH}-${version}.zip`))
        .pipe(gulp.dest(BUILD_DIR + '/' + BRANCH));
};
