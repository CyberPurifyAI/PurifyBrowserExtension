...
const validateChecksum = (url, body) => {
  ...
  if (!checksumMatch[1]) {
    logs.error(
      `Filter rules from ${url.url} doesn't contain a checksum ${partOfResponse}`
    );
  }

  const bodyChecksum = crypto
    .createHash("md5")
    .update(normalizeResponse(body))
    .digest("base64")
    .replace(/=/g, "");

  if (bodyChecksum !== checksumMatch[1]) {
    logs.error(
      `Wrong checksum: found ${bodyChecksum}, expected ${checksumMatch[1]}`
    );
  }
