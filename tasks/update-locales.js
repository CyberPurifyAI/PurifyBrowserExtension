...
import { LOCALES_DIR, LOCALES_DOWNLOAD_URL } from "./consts";
...
const downloadLocales = async () => {
  const localeUrlPairs = locales.map((locale) => {
    ...
    const downloadUrl = `${LOCALES_DOWNLOAD_URL}?${getQueryString(
      crowdinLocale
    )}`;
  ...
  return promiseBatchMap(
    localeUrlPairs,
    LOCALES_DOWNLOAD_BATCH_SIZE,
    async (localeUrlPair) => {
...
const checkRequiredFields = (locale, messages, baseMessages) => {
  ...
  requiredFields.forEach((requiredField) => {
    ...
    if (!fieldData) {
      log.info(
        `"${locale}" locale does't have required field: "${requiredField}"`
      );
