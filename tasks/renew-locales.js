...
const configuration = {
  ...
  targets: ["./src/"], // Directory to search occurrences
...
const getFilesPathsList = async (targets, filesReg) => {
  ...
  return Promise.all(filesListsPromises).then((filesLists) => {
...
const chooseMessagesFromFiles = async (messages, targets, filesReg) => {
  ...
  return Promise.all(filteredMessages).then((messages) => {
    return [
      ...messages.reduce((unique, messageArray) => {
