...
const firefoxWebext = () =>
  gulp
    .src([paths.webkitFiles, paths.chromeFiles, paths.firefox_webext])
...
const preprocess = (done) =>
  preprocessAll(
    paths.dest,
    { browser: FIREFOX_WEBEXT, remoteScripts: true },
    done
  );

// change the extension name based on a type of a build (dev, beta or release)
const localesProcess = (done) =>
  updateLocalesMSGName(BRANCH, paths.dest, done, FIREFOX_WEBEXT, true);
...
const updateManifest = (done) => {
  ...
  switch (process.env.NODE_ENV) {
    ...
    default:
      throw new Error(
        `This task used only in dev and beta builds and received: ${process.env.NODE_ENV}`
      );
  }
...
const createArtifact = async (done) => {
  ...
  const credentialsContent = await cryptor(
