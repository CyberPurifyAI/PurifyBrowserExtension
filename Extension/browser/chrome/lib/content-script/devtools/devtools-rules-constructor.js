...
var DevToolsRulesConstructor = (function () {
   ...
   * @returns {string}
   */
  var makeDefaultCssFilter = function (
    element,
    classList,
    excludeTagName,
    excludeId
  ...
  var constructUrlBlockRuleText = function (
    element,
    urlBlockAttribute,
    oneDomain,
    domain
    ...
    var blockUrlRuleText = urlBlockAttribute.replace(
      /^http:\/\/(www\.)?/,
      "||"
    );
    ...
    if (!oneDomain) {
      blockUrlRuleText =
        blockUrlRuleText + RULE_OPTIONS_MARK + "domain=" + domain;
    }
  ...
  /**
   * Constructs purify rule text from element node and specified options
   *
   * var options = {
  ...
  api.constructRuleText = function (element, options) {
    ...
    if (ruleType === "URL") {
      var blockUrlRuleText = constructUrlBlockRuleText(
        element,
        options.urlMask,
