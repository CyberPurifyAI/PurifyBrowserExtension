/**
 * ----------------------------------------------------------------------------------
 * PurifyBrowserExtension tabs.js
...
purify.windowsImpl = (function (purify) {
  ...
  function toWindowFromChromeWindow(chromeWin) {
    return {
      ...
      type:
        chromeWin.type === "normal" || chromeWin.type === "popup"
          ? chromeWin.type
          : "other",
    };
  }
  ...
  var forEachNative = function (callback) {
    ...
    // https://github.com/CyberPurify/PurifyBrowserExtension/issues/569
...
purify.tabsImpl = (function (purify) {
  ...
  function focusWindow(tabId, windowId, callback) {
     ...
     * https://github.com/CyberPurify/PurifyBrowserExtension/issues/675
     */
  ...
  var activate = function (tabId, callback) {
    ...
    browser.tabs.update(tabIdToInt(tabId), { active: true }, function (
      chromeTab
    ) {
  ...
  var reload = function (tabId, url) {
    if (url) {
      if (purify.utils.browser.isEdgeBrowser()) {
         ...
         * https://github.com/CyberPurify/PurifyBrowserExtension/issues/580
         */
        setTimeout(function () {
  ...
  var sendMessage = function (tabId, message, responseCallback, options) {
    ...
    (browser.tabs.sendMessage || browser.tabs.sendRequest)(
      tabIdToInt(tabId),
      message,
      responseCallback
    );
   ...
   */
  const insertCssCode = !browser.tabs.insertCSS
    ? undefined
    : function (tabId, requestFrameId, code) {
        const injectDetails = {
          code,
   ...
   */
  const executeScriptCode = !browser.tabs.executeScript
    ? undefined
    : function (tabId, requestFrameId, code) {
