/**
 * ----------------------------------------------------------------------------------
 * PurifyBrowserExtension test-content-filter.js
...
QUnit.test("testContentRuleExceptions", function (assert) {
  ...
  assert.ok(
    contentFilter.getMatchedElements(doc, "google.com").indexOf(element) >= 0
  );
...
QUnit.test("testWildcardShortcut", function (assert) {
  ...
  assert.equal(
    "longest string",
    new Wildcard("*tarara*trtr*this is?longest string").shortcut
  );
  assert.equal(
    "longest string",
    new Wildcard("*tarara*trtr*this is?longest string*").shortcut
  ...
  assert.ok(
    new Wildcard("*TEST ?STRING*").matches("tatatatataTEST _STRINGbabababah")
  );
  assert.notOk(
    new Wildcard("*TEST ?STRING*").matches("tatatatataTEST STRINGbabababah")
  );
});
