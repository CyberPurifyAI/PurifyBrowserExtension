...
var ExtendedCss = (function () {
  ...
  utils.isSameOrigin = function (urlA, locationB, domainB) {
    ...
    if (
      locationA.protocol === "javascript:" ||
      locationA.href === "about:blank"
    ) {
      return true;
    }
    ...
    if (!Sizzle) {
      ...
      Sizzle = (function (window) {
        ...
        var sortTokenGroups = (function () {
          ...
          var isSortable = function isSortable(tokens) {
            ...
            while (iTokens--) {
              ...
              if (
                token.type === "PSEUDO" &&
                POSITIONAL_PSEUDOS.indexOf(token.matches[0]) !== -1
              ) {
                return false;
              }
        ...
        tokenize = Sizzle.tokenize = function (selector, parseOnly, options) {
          ...
          return (returnUnsorted
            ? tokensCacheItem.groups
            : tokensCacheItem.sortedGroups
          ).slice(0);
        };
  ...
  var StylePropertyMatcher = (function (window) {
    ...
    var getMatchedCSSRules = useFallback
      ? window.getMatchedCSSRules.bind(window)
      : null;
    /**
    ...
    var getComputedStylePropertyValue = function getComputedStylePropertyValue(
      element,
      pseudoElement,
      propertyName
    ) {
    ...
    var Matcher = function Matcher(propertyFilter, pseudoElement) {
      ...
      try {
        ...
        utils.logError(
          "StylePropertyMatcher: invalid match string ".concat(propertyFilter)
        );
      }
    };
    /**
    ...
    Matcher.prototype.matches = function (element) {
      ...
      var value = getComputedStylePropertyValue(
        element,
        this.pseudoElement,
        this.propertyName
      );
    ...
    var extendSizzle = function extendSizzle(sizzle) {
      ...
      sizzle.selectors.pseudos["matches-css"] = sizzle.selectors.createPseudo(
      ...
      sizzle.selectors.pseudos[
        "matches-css-before"
      ] = sizzle.selectors.createPseudo(function (propertyFilter) {
  ...
  var ExtendedSelectorFactory = (function () {
    ...
    function initialize() {
      ...
      var containsPseudo = Sizzle.selectors.createPseudo(function (text) {
        if (/^\s*\/.*\/[gmisuy]*\s*$/.test(text)) {
          ...
          try {
            ...
            throw new Error(
              "Invalid argument of :contains pseudo class: ".concat(text)
            );
          }
      ...
      Sizzle.selectors.pseudos["if-not"] = Sizzle.selectors.createPseudo(
        function (selector) {
          if (typeof selector === "string") {
            Sizzle.compile(selector);
          }
    ...
    function registerParserOnlyTokens() {
      Sizzle.selectors.pseudos["xpath"] = Sizzle.selectors.createPseudo(
        function (selector) {
          try {
            document.createExpression(selector, null);
          } catch (e) {
            throw new Error(
              "Invalid argument of :xpath pseudo class: ".concat(selector)
            );
          }
      ...
      Sizzle.selectors.pseudos["nth-ancestor"] = Sizzle.selectors.createPseudo(
        function (selector) {
      ...
      Sizzle.selectors.pseudos["upward"] = Sizzle.selectors.createPseudo(
        function (input) {
          if (input === "") {
            throw new Error(
              "Invalid argument of :upward pseudo class: ".concat(input)
            );
          } else if (
      ...
      Sizzle.selectors.pseudos["remove"] = Sizzle.selectors.createPseudo(
        function (input) {
          if (input !== "") {
            throw new Error(
              "Invalid argument of :remove pseudo class: ".concat(input)
            );
          }
    ...
    ExtendedSelectorParser.prototype = {
      ...
      isLastToken: function isLastToken(tokens, i) {
        ...
        var isNextRemoveToken =
          tokens[i + 1] &&
          tokens[i + 1].type === "PSEUDO" &&
          tokens[i + 1].matches &&
          tokens[i + 1].matches[0] === "remove"; // check if the token is last
    ...
    BaseLastArgumentSelector.prototype = {
       ...
       * @param {Array} result
       */
      searchResultNodes: function searchResultNodes(
        node,
        pseudoClassArg,
        result
      ) {
    ...
    XpathSelector.prototype.searchResultNodes = function (
      node,
      pseudoClassArg,
      result
    ) {
    ...
    UpwardSelector.prototype = Object.create(
      BaseLastArgumentSelector.prototype
    );
    ...
    UpwardSelector.prototype.searchResultNodes = function (
      node,
      upwardSelector,
      result
    ) {
    ...
    function RemoveSelector(selectorText, hasValidRemovePart, debug) {
      ...
      BaseLastArgumentSelector.call(
        this,
        modifiedSelectorText,
        hasValidRemovePart,
        debug
    ...
    RemoveSelector.prototype = Object.create(
      BaseLastArgumentSelector.prototype
    );
        ...
        case "+": {
          simpleNodes.forEach(function (node) {
            ...
            Object.values(parentNode.children).forEach(function (childNode) {
              if (
                _this2.matches(childNode) &&
                childNode.previousElementSibling === node
              ) {
                resultNodes.push(childNode);
              }
            });
        ...
        case "~": {
          simpleNodes.forEach(function (node) {
            ...
            Object.values(parentNode.children).forEach(function (childNode) {
              if (
                _this2.matches(childNode) &&
                node.compareDocumentPosition(childNode) === 4
              ) {
                resultNodes.push(childNode);
              }
            });
    ...
    return {
      ...
      createSelector: function createSelector(selector, tokens, debug) {
        return new ExtendedSelectorParser(
          selector,
          tokens,
          debug
        ).createSelector();
    ...
    Parser.prototype = {
      ...
      parseCss: function parseCss() {
        ...
        while (this.cssText) {
          ...
          if (
            this.cssText.charCodeAt(this.nextIndex) !== 123 ||
            /* charCode of '{' */
          ...
          for (var i = 0, l = selectorData.length; i < l; i++) {
            ...
            try {
              var extendedSelector = ExtendedSelectorFactory.createSelector(
                data.selectorText,
                data.groups,
                debug
              );
              ...
              if (
                extendedSelector.pseudoClassArg &&
                extendedSelector.isRemoveSelector
              ) {
  ...
  function ExtendedCss(configuration) {
    ...
    var EventTracker = (function () {
      ...
      return {
        isIgnoredEventType: function isIgnoredEventType() {
          return (
            ignoredEventTypes.indexOf(getLastEventType()) > -1 &&
            getTimeSinceLastEvent() < LAST_EVENT_TIMEOUT_MS
          );
        },
      };
    })();
    ...
    function protectStyleAttribute(node, rules) {
      ...
      var protectionObserver = new utils.MutationObserver(
        createProtectionFunction(styles)
      );
    ...
    var applyRulesScheduler = new utils.AsyncWrapper(
      applyRules,
      APPLY_RULES_DELAY
    );
  ...
  ExtendedCss.query = function (selectorText, noTiming) {
    ...
    try {
      return ExtendedSelectorFactory.createSelector(
        selectorText
      ).querySelectorAll();
    } finally {
      var end = now();
