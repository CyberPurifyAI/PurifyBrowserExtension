    ...
    return new Promise((resolve, reject) => {
      const onRequestLoad = (response) => {
        ...
        if (!isLocal(response.responseURL)) {
          const responseContentType = response.getResponseHeader(
            "Content-Type"
          );
          if (
            !responseContentType ||
            ...
            reject(
              new Error(`Response content type should be: "${contentType}"`)
            );
          }
        }
          ...
          reject(
            new Error(
              `Request error happened: ${
                request.statusText || "status text empty"
              }`
            )
          );
        request.onabort = () =>
          reject(
            new Error(
              `Request was aborted with status text: ${request.statusText}`
            )
          );
        request.ontimeout = () =>
          reject(
            new Error(
              `Request timed out with status text: ${request.statusText}`
            )
          );
 ...
 * condition - just like in some popular programming languages, pre-processor conditions are based on constants declared by ad blockers. Explicit content blocker authors define on their own what exact constants do they declare.
...
const FilterDownloader = (() => {
  ...
  const resolveExpression = (expression, definedProperties) => {
    ...
    const openBracketIndex = expression.lastIndexOf(
      CONDITION_BRACKET_OPEN_CHAR
    );
    if (openBracketIndex !== -1) {
      const endBracketIndex = expression.indexOf(
        CONDITION_BRACKET_CLOSE_CHAR,
        openBracketIndex
      );
      ...
      const resolvedInner =
        expression.substring(0, openBracketIndex) +
        innerResult +
        expression.substring(endBracketIndex + 1);
    ...
    if (indexOfOrOperator !== -1) {
      result =
        ...
        resolveExpression(
          expression.substring(
            indexOfOrOperator + CONDITION_OPERATOR_OR.length,
            expression.length
          ),
          definedProperties
        );
      ...
      result =
        ...
        resolveExpression(
          expression.substring(
            indexOfAndOperator + CONDITION_OPERATOR_AND.length,
            expression.length
          ),
          definedProperties
        );
      ...
      result = !resolveExpression(
        expression.substring(CONDITION_OPERATOR_NOT.length),
        definedProperties
      );
    } else {
  ...
  const resolveCondition = (directive, definedProperties) => {
    const expression = directive
      .substring(CONDITION_DIRECTIVE_START.length)
      .trim();
  ...
  const resolveConditions = (rules, definedProperties) => {
    ...
    for (let i = 0; i < rules.length; i++) {
      ...
      if (rule.indexOf(CONDITION_DIRECTIVE_START) === 0) {
        ...
        if (endLineIndex === -1) {
          throw new Error(
            "Invalid directives: Condition end not found: " + rule
          );
        }
        ...
        if (conditionValue) {
          ...
          result = result.concat(
            resolveConditions(rulesUnderCondition, definedProperties)
          );
        }
        ...
        throw new Error(
          "Invalid directives: Found unexpected condition end: " + rule
        );
      } else {
        result.push(rule);
      }
  ...
  const compile = (rules, filterOrigin, definedProperties) => {
    try {
      ...
      const resolvedConditionsResult = resolveConditions(
        rules,
        definedProperties
      );
  ...
  const downloadFilterRules = (url, filterUrlOrigin, definedProperties) => {
    if (
      REGEXP_ABSOLUTE_URL.test(url) ||
      REGEXP_ABSOLUTE_URL.test(filterUrlOrigin)
    ) {
  ...
  const externalDownload = (url, filterUrlOrigin, definedProperties) => {
    ...
    return FileDownloadWrapper.getExternalFile(
      url,
      filterUrlOrigin,
      definedProperties
    ).then((lines) => {
  ...
  const getLocalFile = (url, filterUrlOrigin, definedProperties) => {
    ...
    return FileDownloadWrapper.getLocalFile(
      url,
      filterUrlOrigin,
      definedProperties
    ).then((lines) => {
