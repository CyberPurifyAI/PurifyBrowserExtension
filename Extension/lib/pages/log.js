...
const STEALTH_ACTIONS_NAMES = {
  ...
  STRIPPED_TRACKING_URL: i18n.getMessage(
...
const RequestWizard = (function () {
  ...
  const splitToPatterns = function (requestUrl, domain, whitelist) {
    ...
    if (`${domain}/` !== url) {
      // Don't duplicate: ||example.com/ and ||example.com^
  ...
  const initCreateRuleDialog = function (
    frameInfo,
    template,
    patterns,
    filteringEvent
    ...
    const rulePatterns = [].slice.call(
      template.querySelectorAll('[name="rulePattern"]')
    );
    const ruleDomainCheckbox = template.querySelector('[name="ruleDomain"]');
    const ruleImportantCheckbox = template.querySelector(
      '[name="ruleImportant"]'
    );
    const ruleMatchCaseCheckbox = template.querySelector(
      '[name="ruleMatchCase"]'
    );
    const ruleThirdPartyCheckbox = template.querySelector(
      '[name="ruleThirdParty"]'
    );
    const ruleTextEl = template.querySelector('[name="ruleText"]');

    ruleDomainCheckbox.setAttribute("id", "ruleDomain");
    ruleDomainCheckbox.parentNode
      .querySelector("label")
      .setAttribute("for", "ruleDomain");
    if (!frameDomain) {
      ruleDomainCheckbox.closest(".checkbox").style.display = "none";
    }

    ruleImportantCheckbox.setAttribute("id", "ruleImportant");
    ruleImportantCheckbox.parentNode
      .querySelector("label")
      .setAttribute("for", "ruleImportant");
    if (
      filteringEvent.requestRule &&
      (filteringEvent.requestRule.whiteListRule ||
        filteringEvent.requestRule.isImportant)
    ) {
      ruleImportantCheckbox.setAttribute("checked", "checked");
    }

    ruleMatchCaseCheckbox.setAttribute("id", "ruleMatchCase");
    ruleMatchCaseCheckbox.parentNode
      .querySelector("label")
      .setAttribute("for", "ruleMatchCase");

    ruleThirdPartyCheckbox.setAttribute("id", "ruleThirdParty");
    ruleThirdPartyCheckbox.parentNode
      .querySelector("label")
      .setAttribute("for", "ruleThirdParty");
    ...
    if (
      filteringEvent.requestRule &&
      filteringEvent.requestRule.documentLevelRule
    ) {
      const optionsField = template.querySelector(".filtering-modal-options");
      if (optionsField) {
        optionsField.style.display = "none";
    ...
    function updateRuleText() {
      const urlPattern = template.querySelector('[name="rulePattern"]:checked')
        .value;
      ...
      if (filteringEvent.requestUrl === "content-security-policy-check") {
        mandatoryOptions = [
          UrlFilterRule.WEBRTC_OPTION,
          UrlFilterRule.WEBSOCKET_OPTION,
        ];
      }
  ...
  const createExceptionCssRule = function (rule, event) {
    ...
    if (ruleText.indexOf(FilterRule.MASK_CSS_INJECT_RULE) > -1) {
      return (
        domainPart +
        generateExceptionRule(ruleText, FilterRule.MASK_CSS_INJECT_RULE)
      );
    }
    if (ruleText.indexOf(FilterRule.MASK_CSS_EXTENDED_CSS_RULE) > -1) {
      return (
        domainPart +
        generateExceptionRule(ruleText, FilterRule.MASK_CSS_EXTENDED_CSS_RULE)
      );
    }
    ...
    if (ruleText.indexOf(FilterRule.MASK_CSS_RULE) > -1) {
      return (
        domainPart + generateExceptionRule(ruleText, FilterRule.MASK_CSS_RULE)
      );
    }
  };
  ...
  const createExceptionScriptRule = function (rule, event) {
    ...
    if (ruleText.indexOf(FilterRule.MASK_SCRIPT_RULE) > -1) {
      return (
        domainPart +
        generateExceptionRule(ruleText, FilterRule.MASK_SCRIPT_RULE)
      );
    }
    if (ruleText.indexOf(FilterRule.MASK_SCRIPT_RULE_UBO) > -1) {
      return (
        domainPart +
        generateExceptionRule(ruleText, FilterRule.MASK_SCRIPT_RULE_UBO)
      );
    }
  };
  ...
  const showCreateExceptionRuleModal = function (frameInfo, filteringEvent) {
    ...
    if (filteringEvent.element) {
      patterns = [
        createExceptionCssRule(filteringEvent.requestRule, filteringEvent),
      ];
    }

    if (filteringEvent.cookieName) {
      patterns = [
        createExceptionCookieRule(filteringEvent.requestRule, filteringEvent),
      ];
    }

    if (filteringEvent.script) {
      patterns = [
        createExceptionScriptRule(filteringEvent.requestRule, filteringEvent),
      ];
    }
  ...
  const getFilterName = function (filterId) {
    ...
    const filterMetadata = filtersMetadata.filter(
  ...
  const showRequestInfoModal = function (frameInfo, filteringEvent) {
    ...
    template.querySelector(
      '[attr-text="requestType"]'
    ).textContent = getRequestType(filteringEvent.requestType);
    template.querySelector('[attr-text="frameDomain"]').textContent = getSource(
      filteringEvent.frameDomain
    );
    if (!filteringEvent.frameDomain) {
      template
        .querySelector('[attr-text="frameDomain"]')
    ...
    if (
      ...
      if (requestRule.filterId !== AntiBannerFiltersId.WHITE_LIST_FILTER_ID) {
        const requestRuleNode = template.querySelector(
          '[attr-text="requestRule"]'
        );
        ...
        template
          .querySelector('[attr-text="requestRule"]')
          .closest("li").style.display = "none";
      }
      template
        .querySelector('[attr-text="replaceRules"]')
        .closest("li").style.display = "none";
      template.querySelector(
        '[attr-text="requestRuleFilter"]'
      ).textContent = getFilterName(requestRule.filterId);
    } else {
      template
        .querySelector('[attr-text="requestRule"]')
        .closest("li").style.display = "none";
      template
        .querySelector('[attr-text="requestRuleFilter"]')
        .closest("li").style.display = "none";
    }
    ...
    if (replaceRules) {
      ...
      if (replaceRules.length > 0) {
        template.querySelector(
          '[attr-text="replaceRules"]'
        ).textContent = replaceRules
    ...
    if (stealthActions) {
      template.querySelector(
        '[attr-text="stealthActions"]'
      ).textContent = getStealthActionNames(stealthActions).join("\r\n");
    } else {
      template
        .querySelector('[attr-text="stealthActions"]')
        .closest("li").style.display = "none";
    }
    ...
    const removeWhiteListDomainButton = template.querySelector(
      "#removeWhiteListDomain"
    );
    const removeUserFilterRuleButton = template.querySelector(
    ...
    if (!requestRule) {
      ...
      requestRule.filterId === AntiBannerFiltersId.WHITE_LIST_FILTER_ID
    ) {
  ...
  const initRequestWizard = function () {
    ...
    createBlockRuleTemplate = document.querySelector(
      "#modal-create-block-rule"
    );
    createExceptionRuleTemplate = document.querySelector(
      "#modal-create-exception-rule"
    );
  };
...
PageController.prototype = {
  init() {
    ...
    this.tabSelector.addEventListener("change", (e) => {
      document.location.hash = `#${e.target.selectedOptions[0].getAttribute(
        "data-tab-id"
      )}`;
    });
    ...
    this.logTable.addEventListener("click", (e) => {
      ...
      if (filteringEvent) {
        contentPage.sendMessage(
          { type: "getTabFrameInfoById", tabId: this.currentTabId },
          (response) => {
  ...
  onTabUpdated(tabInfo) {
    const item = this.tabSelector.querySelector(
  ...
  onTabClose(tabInfo) {
    const element = this.tabSelector.querySelector(
      `[data-tab-id="${tabInfo.tabId}"]`
    );
    if (!element) {
      return;
  ...
  onSelectedTabChange() {
    let selectedItem = this.tabSelector.querySelector(
      `[data-tab-id="${this.currentTabId}"]`
    );
    if (!selectedItem) {
  ...
  _bindSearchFilters() {
    ...
    document
      .querySelector('[name="searchEventRequest"]')
    ...
    const searchEventTypeItems = [].slice.call(
      document.querySelectorAll(".searchEventType")
    );
  ...
  _renderEventsForTab(tabId) {
    ...
    contentPage.sendMessage(
      { type: "getFilteringInfoByTabId", tabId },
      (response) => {
  ...
  _renderTemplate(event) {
    ...
    if (event.requestRule && event.requestRule.filterId !== undefined) {
      event.filterName = RequestWizard.getFilterName(
        event.requestRule.filterId
      );
    ...
    if (event.requestRule) {
      if (
        event.requestRule.filterId === AntiBannerFiltersId.WHITE_LIST_FILTER_ID
      ) {
    ...
    if (event.requestThirdParty) {
      thirdPartyDetails =
        '<img src="images/chain-link.svg" class="icon-chain"><small>Third party</small>';
    }
  ...
  _handleEventShow(el) {
    ...
    let show =
      ...
      StringUtils.containsIgnoreCase(
        filterData.requestUrl,
        this.searchRequest
      ) ||
      StringUtils.containsIgnoreCase(filterData.element, this.searchRequest) ||
      StringUtils.containsIgnoreCase(
        filterData.cookieName,
        this.searchRequest
      ) ||
      StringUtils.containsIgnoreCase(
        filterData.cookieValue,
        this.searchRequest
      );

    if (filterData.requestRule && filterData.requestRule.ruleText) {
      show |= StringUtils.containsIgnoreCase(
        filterData.requestRule.ruleText,
        this.searchRequest
      );
    }
    ...
    show &=
      this.searchTypes.length === 0 ||
      this.searchTypes.indexOf(filterData.requestType) >= 0;

    let checkboxes = true;
    checkboxes &=
      !this.searchWhitelisted ||
      (filterData.requestRule && filterData.requestRule.whiteListRule);
    checkboxes &=
      !this.searchBlocked ||
      (filterData.requestRule && !filterData.requestRule.whiteListRule);
