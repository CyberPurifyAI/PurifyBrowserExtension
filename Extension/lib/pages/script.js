/**
 * ----------------------------------------------------------------------------------
 * PurifyBrowserExtension script.js
...
function customizePopupFooter(isMacOs) {
  // fix title
  let messageId = isMacOs
    ? "thankyou_want_full_protection_mac"
    : "thankyou_want_full_protection";
  let title = document.querySelector(
    ".thanks-prefooter .thanks-prefooter-title"
  );
  ...
  title = document.querySelector(
    ".thanks-prefooter .thanks-prefooter-table .tpt-head-full"
  );
  i18n.translateElement(title, messageId);
...
function createEventListener(events, callback, onUnloadCallback) {
  ...
  contentPage.sendMessage(
    { type: "addEventListener", events: events },
    function (response) {
