...
(function (purify, api) {
   ...
   * http://cyberpurify.com/en/filterrules.html#cssInjection
   */
  var CssFilterRule = (function () {
    ...
    var parsePseudoClass = function (selector) {
      ...
      while (squareBracketIndex >= 0) {
        ...
        if (
          nameStartIndex > 0 &&
          selector.charAt(nameStartIndex - 1) === "\\"
        ) {
        ...
        while (squareBracketIndex >= 0) {
          if (nameStartIndex > squareBracketIndex) {
            var squareEndBracketIndex = selector.indexOf(
              "]",
    ...
    var constructor = function (rule, filterId) {
      ...
      var mask = api.FilterRule.findRuleMarker(
        rule,
      ...
      if (isElemhideRule) {
        ...
        // https://github.com/CyberPurify/PurifyBrowserExtension/issues/1624
        if (
          /{.+}/.test(cssContent) ||
      ...
      if (!isInjectRule) {
        ...
        // https://github.com/CyberPurify/PurifyForAndroid/issues/701
        const pseudoClass = parsePseudoClass(cssContent);
        if (pseudoClass !== null) {
          if (
            CssFilterRule.SUPPORTED_PSEUDO_CLASSES.indexOf(pseudoClass.name) < 0
          ) {
      ...
      if (isInjectRule) {
        ...
        if (!/{.+}/.test(cssContent)) {
          throw new Error(
            `Invalid css injection rule, no style presented: ${rule}`
          );
        }
        // discard css inject rules containing "url"
        // https://github.com/CyberPurify/PurifyBrowserExtension/issues/1196
   ...
   */
  CssFilterRule.SUPPORTED_PSEUDO_CLASSES = [
    ":active",
    ":checked",
    ":contains",
    ":disabled",
   ...
   */
  CssFilterRule.EXTENDED_CSS_MARKERS = [
    "[-ext-has=",
    "[-ext-contains=",
    "[-ext-has-text=",
    "[-ext-matches-css=",
