/**
 * ----------------------------------------------------------------------------------
 * PurifyBrowserExtension content-filter-rule.js
...
(function (purify, api) {
  ...
  var ContentFilterRule = function (ruleText, filterId) {
    ...
    while (ruleStartIndex !== -1) {
      var equalityIndex = ruleText.indexOf(
        api.FilterRule.EQUAL,
        ruleStartIndex + 1
      );
      ...
      var ruleEndIndex = ruleText.indexOf(
        ATTRIBUTE_END_MARK,
        quoteEndIndex + 1
      );

      var attributeName = ruleText.substring(ruleStartIndex + 1, equalityIndex);
      var attributeValue = ruleText.substring(
        quoteStartIndex + 1,
        quoteEndIndex
      );
  ...
  ContentFilterRule.prototype.searchForParentElement = function (element) {
    ...
    for (var i = 0; i < this.parentSearchLevel; i++) {
      ...
      if (
        this.parentElements.indexOf(parentElement.tagName.toLowerCase()) > 0
      ) {
