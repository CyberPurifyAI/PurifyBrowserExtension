/**
 * ----------------------------------------------------------------------------------
 * PurifyBrowserExtension url-filter-rule.js
...
(function (purify, api) {
        ...
        if (c === UrlFilterRule.OPTIONS_DELIMITER) {
          if (i > 0 && ruleText.charAt(i - 1) === ESCAPE_CHARACTER) {
            ...
            if (foundEscaped) {
              ...
              const search = api.SimpleRegex.escapeRegExp(
  ...
  function containsOption(options, option) {
    return options !== null && (options & option) === option;
  }
  ...
  var UrlFilterRule = function (rule, filterId) {
    ...
    if (this.isRegexRule) {
      ...
      if (
        !this.isCookieRule() &&
        UrlFilterRule.ANY_URL_REGEX === regexp &&
  ...
  UrlFilterRule.prototype.getUrlRegExpSource = function () {
    if (!this.urlRegExpSource) {
      ...
      this.urlRegExpSource = api.SimpleRegex.createRegexText(
        parseResult.urlRuleText
      );
    }
  ...
  UrlFilterRule.prototype.isFiltered = function (
    ...
    if (
      this.shortcut !== null &&
      requestUrl.toLowerCase().indexOf(this.shortcut) === -1
    ) {
      return false;
    }
  ...
  const extractOptionsAndValues = (options) => {
    ...
    return optionsParts.reduce((acc, optionPart) => {
      ...
      let optionName =
        valueIndex >= 0 ? optionPart.substr(0, valueIndex) : optionPart;
      const optionValue =
        valueIndex >= 0 ? optionPart.substr(valueIndex + 1) : "";
    ...
    for (let i = 0; i < optionsParts.length; i += 1) {
      ...
      switch (optionName) {
        ...
        case UrlFilterRule.POPUP_OPTION:
          this._setUrlFilterRuleOption(
            UrlFilterRule.options.BLOCK_POPUPS,
            true
          );
          break;
        ...
        case UrlFilterRule.BADFILTER_OPTION:
          this.badFilter = this.ruleText
            ...
            .replace(
              api.FilterRule.COMA_DELIMITER + UrlFilterRule.BADFILTER_OPTION,
              ""
            )
            .replace(
        ...
        default:
          ...
          if (optionName in UrlFilterRule.contentTypes) {
            ...
            optionName[0] === api.FilterRule.NOT_MARK &&
   ...
   * @param contentType Content type to append
   */
  UrlFilterRule.prototype._appendRestrictedContentType = function (
    contentType
  ) {
