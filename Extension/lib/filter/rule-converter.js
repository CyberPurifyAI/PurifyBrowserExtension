/**
 * ----------------------------------------------------------------------------------
 * PurifyBrowserExtension rule-converter.js
...
(function (purify, api) {
  ...
  /**
   * CyberPurify scriptlet mask
   */
  // eslint-disable-next-line no-template-curly-in-string
  const PURIFY_SCRIPTLET_MASK = "${domains}#%#//scriptlet(${args})";

  /**
   * CyberPurify scriptlet exception mask
   */
  // eslint-disable-next-line no-template-curly-in-string
  const PURIFY_SCRIPTLET_EXCEPTION_MASK = "${domains}#@%#//scriptlet(${args})";
  ...
  /**
   * CyberPurify max-length tag for uBlock scripts conversion
   */
  const PURIFY_SCRIPT_MAX_LENGTH = '[max-length="262144"]';
  /**
  ...
  /**
   * Convert string of UBO scriptlet rule to CyberPurify scritlet rule
  ...
  function convertUboScriptletRule(rule) {
    ...
    let template;
    if (mask.indexOf("@") > -1) {
      template = PURIFY_SCRIPTLET_EXCEPTION_MASK;
    } else {
      template = PURIFY_SCRIPTLET_MASK;
    }
    ...
    return replacePlaceholders(template, { domains, args });
  }

  /**
   * Convert string of ABP scriptlet rule to CyberPurify scritlet rule
   * @param {string} rule UBO scriptlet rule
   */
  function convertAbpSnippetRule(rule) {
    ...
    const template =
      mask === ABP_SCRIPTLET_MASK
        ? PURIFY_SCRIPTLET_MASK
        : PURIFY_SCRIPTLET_EXCEPTION_MASK;
  ...
  function convertUboScriptTagRule(ruleText) {
    ...
    for (let i = 1; i < match.length; i += 1) {
      ...
      if (isRegExp(attr)) {
        ...
        rules.push(
          `${domains}${adgScriptTag}[tag-content="${attr}"]${PURIFY_SCRIPT_MAX_LENGTH}`
        );
      }
    }
  ...
  function convertUboCssStyleRule(ruleText) {
    ...
    const uboMarkers = Object.keys(UBO_CSS_RULE_MARKERS).map(
      (key) => UBO_CSS_RULE_MARKERS[key]
    );
  ...
  function convertOptions(rule) {
    ...
    const cspParts = updatedOptionsParts.filter((optionsPart) =>
      stringUtils.startWith(optionsPart, CSP_OPTION)
    );

    if (cspParts.length > 1) {
      const allButCsp = updatedOptionsParts.filter(
        (optionsPart) => !stringUtils.startWith(optionsPart, CSP_OPTION)
      );
    ...
    if (hasAllOption) {
      ...
      return allOptionReplacers.map((replacers) => {
        ...
        const optionsButAllAndReplacer = updatedOptionsParts.filter(
          (option) => !(replacers.includes(option) || option === ALL_OPTION)
        );

        // try get converted values, used for INLINE_SCRIPT_OPTION, INLINE_FONT_OPTION
        const convertedReplacers = replacers.map(
          (replacer) => conversionMap[replacer] || replacer
        );

        // add replacer to the list of options
        const updatedOptionsString = [
          ...convertedReplacers,
          ...optionsButAllAndReplacer,
        ]
          .filter((entity) => entity)
          .join(",");

        // create a new rule
        return updatedOptionsString.length < 1
          ? domainPart
          : `${domainPart}\$${updatedOptionsString}`;
      });
    }
   ...
   * @return {boolean}
   */
  const isComment = (rule) =>
    stringUtils.startWith(rule, api.FilterRule.COMMENT);

  /**
   * Convert external scriptlet rule to CyberPurify scriptlet syntax
